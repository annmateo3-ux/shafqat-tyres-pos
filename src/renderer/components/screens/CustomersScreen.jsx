    ...
    <div className="space-y-5">
      <div className="card p-4 flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-xs text-t2">Outstanding Balance</div>
      ...
      <div>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {payments.length === 0 ? <p className="text-t2 text-sm">No payments yet</p> : payments.map(p => (
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-t2 text-sm">{customers.length} customers · {fmt.currency(totalDue)} total due</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-4 py-3 text-right text-xs text-t2 font-medium">Balance Due</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="5" className="py-12 text-center text-t2">No customers found</td></tr>
              ) : filtered.map(c => (
                <tr key={c.id} className="table-row">
                  <td className="px-4 py-3 font-medium text-white">{c.name}</td>
                  <td className="px-4 py-3 text-t2">{c.phone || '—'}</td>
                  <td className="px-4 py-3 text-t2 font-mono text-xs">{c.vehicle_plate || '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      {c.balance > 0 && (
                        <button onClick={() => setPayItem(c)} className="p-1.5 hover:bg-card rounded-lg text-t2 hover:text-green-400 transition-colors" title="Record payment">
                          <CreditCard size={14} />
                        </button>
                      )}
                      <button onClick={() => setPayItem(c)} className="p-1.5 hover:bg-card rounded-lg text-t2 hover:text-accent transition-colors"><Eye size={14} /></button>
                      <button onClick={() => { setEditItem(c); setModal('form') }} className="p-1.5 hover:bg-card rounded-lg text-t2 hover:text-accent transition-colors"><Edit size={14} /></button>
                      {isAdmin && <button onClick={() => setDeleteId(c.id)} className="p-1.5 hover:bg-card rounded-lg text-t2 hover:text-red-400 transition-colors"><Trash size={14} /></button>}
                    </div>
                  </td>
                </tr>
              ))}
